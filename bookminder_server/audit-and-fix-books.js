...
if (entriesToDelete.length > 0) {
    ...
    if (shouldFix) {
        ...
        const checkOtherFormatsStmt = db.prepare('SELECT COUNT(*) as count FROM books_formats WHERE book_id = ? AND id != ?');
        ...
        const transaction = db.transaction(() => {
            for (const entry of entriesToDelete) {
                // Check if book has other formats BEFORE deleting this one
                const result = checkOtherFormatsStmt.get(entry.book_id, entry.format_id);
                const willDeleteBook = result.count === 0;
                
                if (willDeleteBook) {
                    // No other formats, delete all related records first, then the book
                    ...
                    // Has other formats, just delete this format
                    deleteFormatStmt.run(entry.format_id);
                    console.log(`  Deleted format ${entry.format_id} for book ${entry.book_id} (has ${result.count} other format(s))`);
                }
                deletedFormats++;
            }
        });
        ...
        try {
            ...
            db.pragma('foreign_keys = ON'); // Re-enable foreign keys even on error
